	...
	public void perform(Socket clientSocket){
		...
		try{ 
			...
			while ((inputLine = in.readLine()) != null) {
				ThreadPool resThread = (ThreadPool)Thread.currentThread();
				...
				if ((inputLine == null) || inputLine.startsWith("JOIN_CHATROOM")) {
					...
					ManageChatRoom.getInstance().handleChatInRoom(inputLine, clientSocket);
				}

				if(roomName != null && clientAddress != null && port != null && clientName != null){
					ManageChatRoom manager = ManageChatRoom.getInstance();
					ChatRoom room = manager.getChatData(roomName, clientSocket);
			...
			System.out.println("Error while receiving or sending data from/to client.");
		} finally{
			try{
				in.close();
				out.close();
